...
class IObjFunction : public rabit::Serializable {
 public:
   ...
   * \param validation val data or not bool
   */
  virtual double Eval(const DType *weight, size_t size, bool validation) = 0;
  /*!
   * \return number of feature dimension to be allocated
   * only called once during initialization
   */
  virtual void InitNumDim(size_t &dim, size_t &size) = 0;
  /*!
...
class LBFGSSolver {
 public:
  ...
  virtual void SetValidation(bool isVal){
    has_val = isVal;
  }
  /*!
  ...
  virtual void Init(void) {
    ...
    if (version == 0) {
      gstate.obj->InitNumDim(gstate.num_dim, gstate.num_size);
    } else {
  ...
  virtual bool UpdateOneIter(void) {
    ...
    // update the validation obj and early nums
    if (has_val) {
        gstate.new_objval_val = this->Eval(g.weight, true);
        if (early_stop > 0) {
          if (gstate.new_objval_val > gstate.best_objval_val)
            early_num ++;
          else {
            early_num = 0;
            gstate.best_objval_val = gstate.new_objval_val;
            memcpy(g.best_weight, g.weight, g.num_dim * sizeof(float));
          }
        }
    }
    ...
    if (gstate.num_iteration > static_cast<size_t>(min_lbfgs_iter)) {
      ...
      if (early_num > early_stop) {
        memcpy(g.weight, g.best_weight, g.num_dim * sizeof(float));
        if (rabit::GetRank() == 0) {
          rabit::TrackerPrintf           
              ("[EARLY-STOP] L-BFGS: early stop at best_val_objval=%g, with validation earlystop=%d\n",
               gstate.best_objval_val,
               early_stop);
        }
        return true;
      }
    ...
    if (silent == 0 && rabit::GetRank() == 0) {
      if (has_val){
        rabit::TrackerPrintf
            ("[%d] L-BFGS: linesearch finishes in %d rounds, new_objval=%g, improvment=%g, val_objval=%g\n",
             gstate.num_iteration, iter,
             gstate.new_objval/gstate.num_size,
             (gstate.old_objval - gstate.new_objval)/gstate.num_size,
             gstate.new_objval_val);
      }
      else{
        rabit::TrackerPrintf
 ...
 protected:
  ...
  inline double Eval(const DType *weight, bool validation = false) {
    double val = gstate.obj->Eval(weight, gstate.num_dim, validation);    
 ...
 private:
  ...
  struct GlobalState : public rabit::Serializable {
   public:
    ...
    // number of samples in the solver
    size_t num_size;
    ...
    // new objective value
    double new_objval;
    // history validation objective value
    double best_objval_val;
    // new validation objective value
    double new_objval_val;
    // objective function
    IObjFunction<DType> *obj;
    // temporal storage
    DType *grad, *weight, *tempw, *best_weight;
    // constructor
    GlobalState(void)
        : obj(NULL), grad(NULL),
          weight(NULL), tempw(NULL), best_weight(NULL) {
      size_memory = 10;
      num_iteration = 0;
      num_dim = 0;
      old_objval = 0.0;
      best_objval_val = std::numeric_limits<float>::max();
      new_objval_val = 0.0;
      offset_ = 0;
    }
    ~GlobalState(void) {
